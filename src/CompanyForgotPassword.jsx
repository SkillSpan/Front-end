...
const CompanyForgotPassword = ({ onBackToLogin }) => {
  const [stage, setStage] = useState('request'); // request -> code -> password -> success
  ...
  // Step 1 of 2: check the code on its own via
  // POST /auth/forgot-password/verify before asking for a new password.
  // ⚠️ Response shape isn't documented (see api.js) - only success/failure
  // is used here.
  const handleVerifyOtp = async (e) => {
    ...
    try {
      ...
      setIsSubmitting(false);
      setErrors({ otp: err.message || 'The verification code is invalid or has expired.' });
    }
  };

  // Step 2 of 2: the code is already confirmed valid - now actually reset
  // the password. We still send the OTP again (see api.js resetPassword)
  // since there's no confirmed reset-token mechanism from the verify call.
      ...
      <div className="cfp-card">
        ...
        <div className="cfp-form-col">
          ...
          <div className="cfp-form-inner">
            ...
            {stage === 'password' && (
              <>
                <h1 className="cfp-heading">Set a New Password</h1>
                <p className="cfp-subtext">
                  Code confirmed. Choose a new password for <strong>{email}</strong>.
                </p>
                ...
                <p className="cfp-switch-text">
                  <span className="cfp-link" onClick={() => setStage('code')}>← Re-enter code</span>
                </p>
              </>
            )}
