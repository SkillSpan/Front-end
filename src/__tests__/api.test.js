...
describe('api.js', () => {
  ...
  it('verifyForgotPasswordOtp posts email + otp to /api/v1/auth/forgot-password/verify', async () => {
    const fetchMock = vi.fn().mockResolvedValue({
      ok: true,
      status: 200,
    ...
    const { verifyForgotPasswordOtp, API_BASE_URL } = await import('../api');
    await verifyForgotPasswordOtp('a@b.com', '123456');

    const [url, options] = fetchMock.mock.calls[0];
    expect(url).toBe(`${API_BASE_URL}/api/v1/auth/forgot-password/verify`);
    expect(JSON.parse(options.body)).toEqual({ email: 'a@b.com', otp: '123456' });
  });

  it('clears the session and notifies onSessionExpired when an authenticated request gets a 401', async () => {
    vi.stubGlobal(
      'fetch',
      vi.fn().mockResolvedValue({
        ok: false,
    ...
    const { getProfile, saveSession, isAuthenticated, onSessionExpired } = await import('../api');
    saveSession({ token: 'expired-token', user: { id: 1 } });
    expect(isAuthenticated()).toBe(true);
    ...
    expect(isAuthenticated()).toBe(false); // session cleared
    expect(handler).toHaveBeenCalledOnce();

    onSessionExpired(null); // don't leak this handler into other tests
  });
