...
function normalizeSkillRow(raw) {
  return {
    ...
    // Backend stores importance_weight as a 0-1 decimal (see
    // career_role_skills migration + CareerRoleSeeder, e.g. 0.9, 0.6, 0.5) -
    // NOT a 0-100 integer. Math.round()-ing that directly used to collapse
    // almost every skill's weight down to 0 or 1, which then either
    // silently dropped the skill from estimateReadiness() below (weight=0
    // gets filtered out) or made every skill count equally regardless of
    // actual importance. Scale to 0-100 first so relative weighting (0.9
    // vs 0.5 vs 0.6) actually survives normalization.
    weight: Math.round(Number(raw.weight ?? raw.importance ?? raw.importance_weight ?? 0) * 100),
...
export async function fetchMySkillLevels() {
  ...
  // Checked against the real SkillsController@matrix: learner_id is NOT
  // actually read from the query string server-side (it's an unbound
  // method parameter, so it silently stays null), meaning this endpoint
  // always returns the WHOLE learner_skills table regardless of what we
  // pass. The client-side filter below is the only thing actually scoping
  // this to the signed-in learner - don't remove it even if the backend
  // starts accepting a learner_id param later, until that's confirmed.
  if (!learnerId) return {};
